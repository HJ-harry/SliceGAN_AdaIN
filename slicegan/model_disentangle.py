...
def train(pth, imtype, datatype, real_data, codes, Disc, Gen, nc, l, nz, sf):
    ...
    train the generator
        ...
        :param codes: list contaning codes which represent grain size
        :param Disc:
        :param Gen:
        :param nc: channels
        :param l: image size
        ...
        :return:
    """
    # Don't think I really need this part for three phase
    if len(real_data) == 1:
        real_data *= 3
        isotropic = True
    else:
    ...
    dataset = preprocessing.batch_multi(real_data, codes, datatype, l, sf)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (datax, datay, dataz, code) in enumerate(dataloader, 1):
            ...
            # Due to the discrepancy btw. generator batch size and discriminator batch size. Easy fix.
            code_D = code[:8, :]
            code_D = code_D.to(device=device)
            fake_data = netG(noise, code_D).detach()
